...
public class ConnectionTest {
    ...
    public static void main(String[] args) throws Exception {
        try {
            /*使用连接池创建100个连接的时间*/
			       /*// 创建数据库连接库对象
			       ConnectionPool connPool = new ConnectionPool("com.mysql.jdbc.Driver","jdbc:mysql://localhost:3306/orion", "root", "123456");
			       // 新建数据库连接库
		           connPool.createPool();*/
            ...
            for (int i = 0; i < 100; i++) {
                ...
                Connection conn = DriverManager.getConnection(
                        "jdbc:mysql://localhost:3306/orion", "root", "123456");
                Statement stmt = conn.createStatement();
                ResultSet rs = stmt.executeQuery(sql);
                while (rs.next()) {
                }
